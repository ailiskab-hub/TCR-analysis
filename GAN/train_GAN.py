...
log_dir = f"logs/gan/{current_time}"
...
b_blos = pd.read_csv('olga_300.csv') #pd.read_pickle('olga_enc.pkl')
b_blos['seq_enc'] = b_blos['b_cdr3aa'].apply(encode_seq)
...
def histo_samples(sample_pgenes, show=True):
    ...
    # # b_preds = torch.argmax(fake_seqs, dim=2)
    # b_preds = fake_seqs.to('cpu')#.detach().numpy()
    # for seq in b_preds:
    #     # print(seq)
    #     _, pgen = decode_seq(seq)
    #     sample_pgenes.append(pgen)
    
    # all_pgenes.append(sample_pgenes)
    # sample_pgenes.append(1e-8)
    if show:
